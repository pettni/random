...
Xn = Nonlinear_Triangulation(K, zeros(3,1), eye(3), C, R, C3, R3, x1, x2, x3, X);

% Display point cloud and 3 camera poses
Display3D({zeros(3,1), C, C3}, {eye(3), R, R3}, Xn);

% Calculate reprojection points
x1p = K * Xn';
x1p = x1p ./ repmat(x1p(3, :), [3, 1]);
x2p = K * R * (Xn' - repmat(C, [1 size(Xn,1)]));
x2p = x2p ./ repmat(x2p(3, :), [3, 1]);
x3p = K * R3 * (Xn' - repmat(C3, [1 size(Xn,1)]));
x3p = x3p ./ repmat(x3p(3, :), [3, 1]);
